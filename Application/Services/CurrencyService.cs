    ...
    public class CurrencyService : ICurrencyService
    {
        ...
        public async Task<decimal> GetExchangeRate(string fromCurrency, string toCurrency)
        {
            ...
            try
            {
                ...
                var response = await client.GetAsync($"latest/{fromCurrency}"); // obtiene cuantas unidades de otras monedas valen 1 unidad de fromCurrency
                response.EnsureSuccessStatusCode(); // asegura que la respuesta sea un 200

                var content = await response.Content.ReadAsStringAsync(); // de toda la respuesta se queda con el context
                ...
                {
                    return exchangeData.rates[toCurrency]; // devuelve la tasa de cambio de fromCurrency a toCurrency
                }
